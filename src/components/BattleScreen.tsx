...
const routeStops = [
  ...
  { left: '67%', bottom: '59%', label: 'Ruína Verde' },
  { left: '76%', bottom: '68%', label: 'Portal das Águas' },
];

export function BattleScreen({ territoryId, profile, onWin, onLeave }: BattleScreenProps) {
  const adventure = territoryStories[territoryId] ?? territoryStories.brasil;
  ...
  return <main className="relative min-h-[100dvh] overflow-hidden bg-[#08172d] text-white">
    ...
    <section className="absolute inset-x-0 bottom-0 top-16 z-10 mx-auto max-w-7xl">
      <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="pointer-events-none absolute inset-0 h-full w-full overflow-visible"><path d="M27 81 C35 74, 37 70, 43 67 S51 58, 56 52 S63 43, 67 36 S72 33, 76 32" fill="none" stroke="rgba(255,222,104,.95)" strokeWidth=".7" strokeDasharray="2.2 1.5" className="drop-shadow-[0_0_7px_rgba(250,204,21,.85)]" /></svg>
      ...
      {[0, 1, 2].map((index) => { const stop = routeStops[index + 1]; return <button key={index} onClick={index === pathStep ? openNextChallenge : undefined} disabled={index !== pathStep} className={`absolute z-30 grid h-12 w-12 place-items-center rounded-full border-4 text-xs font-black shadow-lg transition sm:h-16 sm:w-16 ${index < pathStep ? 'border-emerald-200 bg-emerald-500 text-white' : index === pathStep ? 'border-yellow-100 bg-yellow-400 text-slate-950 shadow-yellow-400/70 animate-pulse' : 'border-slate-300/50 bg-slate-950/75 text-slate-300'}`} style={{ left: stop.left, bottom: stop.bottom, transform: 'translate(-50%, 50%)' }}>{index < pathStep ? <Check className="h-6 w-6" /> : index === pathStep ? <Sparkles className="h-6 w-6" /> : <Lock className="h-5 w-5" />}<span className="absolute -bottom-7 whitespace-nowrap text-[9px] font-black text-white drop-shadow">{adventure.challenges[index].name}</span></button>; })}
      <button onClick={isBoss ? openNextChallenge : undefined} disabled={!isBoss} aria-label={isBoss ? `Abrir ${adventure.portal}` : `${adventure.portal} bloqueado`} className={`absolute z-30 grid h-16 w-16 place-items-center rounded-full border-4 text-cyan-50 shadow-[0_0_30px_rgba(34,211,238,.65)] ${isBoss ? 'border-yellow-100 bg-cyan-400/45 animate-pulse' : 'border-cyan-100 bg-cyan-400/30'}`} style={{ left: routeStops[3].left, bottom: routeStops[3].bottom, transform: 'translate(-50%, 50%)' }}><MapPin className="h-8 w-8" /><span className="absolute -bottom-6 whitespace-nowrap text-[9px] font-black">{adventure.portal}</span></button>
      <img src="/assets/terravox-explorer.webp" alt="Explorador Terravox" className="pointer-events-none absolute z-20 w-[18vw] max-w-[175px] min-w-[92px] drop-shadow-[0_16px_14px_rgba(0,0,0,.7)] transition-all duration-700" style={{ left: routeStops[pathStep].left, bottom: routeStops[pathStep].bottom, transform: 'translate(-45%, 35%)' }} />
